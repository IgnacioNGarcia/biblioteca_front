...
export default function DetalleSocio() {
  ...
  useEffect(() => {
    if (id) {
      const socioEncontrado = sociosMock.find(
        (socio) => socio.id === Number(id)
      );
      setSocio(socioEncontrado || null); // Si no encuentra el libro, se establece null
      const prestamosUsuario = prestamosMock.filter(
        (prestamo) => prestamo.socioId === Number(id)
      );
      ...
      <Paper sx={{ p: 3 }}>
            ...
            <Paper sx={{ p: 2, height: "100%" }}>
              ...
              {editMode ? (
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  ...
                  <Typography>
                    <strong>Apellido:</strong> {socio.apellido}
                  </Typography>
                  <Typography>
                    <strong>Número de socio:</strong> {socio.id}
                  </Typography>
                  <Typography>
                  ...
                  <Typography>
                    <strong>Fecha de Alta:</strong> {socio.fechaAlta}
                  </Typography>
                </Box>
              )}
            ...
            <Paper sx={{ p: 2, height: "100%" }}>
              ...
              >
                ...
                <Button
                  variant="contained"
                  onClick={() =>
                    router.push(`/prestamos/agregar?socioId=${socio.id}`)
                  }
                >
                  Agregar Préstamo
                </Button>
                    ...
                    {prestamosPaginados.map((p) => {
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() =>
                            router.push(
                              `/prestamos/${p.id}?fromSocio=${socio?.id}`
                            )
                          }
                        >
        ...
        <Paper sx={{ p: 2, height: "100%" }}>
          ...
          >
            ...
            <Button
              variant="contained"
              onClick={() => router.push("/cuotas/agregar")}
            >
              Pagar Nueva Cuota
            </Button>
          </Box>
            ...
            <Table size="small">
              ...
              <TableBody>
                {CuotasPorSocioMock.map((c) => {
                  const cuota = CuotasMock.find(
                    (cuota) => cuota.id === c.cuotaId
                  );
                  return (
                    <TableRow key={c.id}>
